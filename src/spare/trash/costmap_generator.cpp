...
CostmapGenerator::CostmapGenerator() : Node("costmap_generator"){
    ...
    laser_scan_sub_ = this->create_subscription<sensor_msgs::msg::LaserScan>(
        "/scan11", rclcpp::SensorDataQoS(), std::bind(&CostmapGenerator::laserScanCallback, this, std::placeholders::_1));

    gps_sub_ = this->create_subscription<sensor_msgs::msg::NavSatFix>("/gps", 10, std::bind(&CostmapGenerator::gpsCallback, this, std::placeholders::_1));

    imu_sub = this->create_subscription<sensor_msgs::msg::Imu>("/imu", 1, std::bind(&CostmapGenerator::imuCallback, this, std::placeholders::_1));

    costmap_pub = this->create_publisher<nav_msgs::msg::OccupancyGrid>("/costmap", custom_qos);

    costmap_pub_raw = this->create_publisher<nav2_msgs::msg::Costmap>("/costmap_raw", custom_qos);

    marker_pub_ = this->create_publisher<visualization_msgs::msg::Marker>("/obstacle_marker", custom_qos);
...
void CostmapGenerator::gpsCallback(const sensor_msgs::msg::NavSatFix::SharedPtr msg)
{
    if (!gps_initialized_) {
        initial_lat = msg->latitude;
        initial_lon = msg->longitude;
        convertGPSToXY(msg->latitude, msg->longitude, initial_x, initial_y);
        gps_initialized_ = true;
        return;
    }

    double delta_x, delta_y;
    convertGPSToXY(msg->latitude, msg->longitude, delta_x, delta_y);
    robot_x = delta_x;
    robot_y = delta_y;
}

void CostmapGenerator::imuCallback(const sensor_msgs::msg::Imu::SharedPtr msg) {
...
void CostmapGenerator::convertGPSToXY(double latitude, double longitude, double &x, double &y)
{
    double lat_rad = latitude * M_PI / 180.0;
    double lon_rad = longitude * M_PI / 180.0;
    double initial_lat_rad = initial_lat * M_PI / 180.0;
    double initial_lon_rad = initial_lon * M_PI / 180.0;

    // 위도, 경도를 미터 단위로 변환 (지구 반경 6378137m, 평면 근사)
    const double earth_radius = 6378137.0;  // 미터 단위 (WGS-84)

    // 위도 및 경도 차이를 거리(m)로 변환 (위도 차이는 항상 일정)
    x = (lon_rad - initial_lon_rad) * earth_radius * cos(initial_lat_rad);  // 경도는 위도에 따라 조정
    y = (lat_rad - initial_lat_rad) * earth_radius;
}
    ...
    while (rclcpp::ok() && !stop_publishing_) {
        double interval = publish_interval_.load(); 
        {
            ...
            if (!latest_data_.empty()) {
                generateCostmap(latest_data_, width, height, resolution);
            }
        }
...
void CostmapGenerator::generateCostmap(const std::vector<int8_t>& data, int width, int height, double resolution) {
    ...
    occupancy_grid_msg->info.origin.position.x = robot_x - (width * resolution / 2.0);
    occupancy_grid_msg->info.origin.position.y = robot_y - (height * resolution / 2.0);
    
    tf2::Quaternion q;
    q.setRPY(0, 0, robot_yaw);  // roll=0, pitch=0, yaw=robot_yaw
    occupancy_grid_msg->info.origin.orientation = tf2::toMsg(q);
    
    occupancy_grid_msg->data = data;
    
    if (Tracker.costmapCallback(occupancy_grid_msg)) {
        Eigen::Vector4f state = Tracker.getState();
        float obstacle_radius = Tracker.getObstacleRadius();
        Updater->DAJ(*occupancy_grid_msg, state, obstacle_radius, 0.3);
        publishObstacleMarker(state);
    }

    publishRawCostmap(occupancy_grid_msg);
    publishCostmap(occupancy_grid_msg);
}

void CostmapGenerator::publishRawCostmap(const std::shared_ptr<nav_msgs::msg::OccupancyGrid>& occupancy_grid_msg) {
    ...
    costmap_pub_raw->publish(*costmap_msg);
}
...
void CostmapGenerator::publishCostmap(const std::shared_ptr<nav_msgs::msg::OccupancyGrid>& occupancy_grid_msg) {
    ...
    costmap_pub->publish(*occupancy_grid_msg);
}
...
//TF 관련 함수
void CostmapGenerator::publishMapToOdomTF()
{
...
void CostmapGenerator::publishOdomToBaseLinkTF()
{
    geometry_msgs::msg::TransformStamped transformStamped;
    transformStamped.header.stamp = this->get_clock()->now();
...
void CostmapGenerator::publishTF()
{
    publishMapToOdomTF();
    publishOdomToBaseLinkTF();
}
