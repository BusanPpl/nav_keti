...
// {
//     if (!map_exist_) {
//         // RCLCPP_WARN(this->get_logger(), "Costmap is not ready yet.");
//         return;
//     }

//     //costmap좌표계에서 연산, 로봇 기준 로컬 좌표계
//     double local_target_x, local_target_y;
//     double local_robot_x, local_robot_y;
//     local_target_x = (target_x - robot_x - current_costmap_.metadata.origin.position.x) / current_costmap_.metadata.resolution;
//     local_target_y = (target_y - robot_y - current_costmap_.metadata.origin.position.y) / current_costmap_.metadata.resolution;
//     local_robot_x = (- current_costmap_.metadata.origin.position.x) / current_costmap_.metadata.resolution;
//     local_robot_y = (- current_costmap_.metadata.origin.position.y) / current_costmap_.metadata.resolution;

//     // 로봇의 목표 위치가 costmap 경계를 벗어나지 않도록 조정
//     if (!clipToCostmapBoundary(local_robot_x, local_robot_y, local_target_x, local_target_y))
//     {
//         // RCLCPP_WARN(this->get_logger(), "Target position adjusted to costmap boundary.");
//     }

//     AStar::Vec2i source = {static_cast<int>(local_robot_x), static_cast<int>(local_robot_y)};
//     AStar::Vec2i target = {static_cast<int>(local_target_x), static_cast<int>(local_target_y)};
