...
CostmapGenerator::CostmapGenerator() : Node("costmap_generator"){
    ...
    laser_scan_sub_ = this->create_subscription<sensor_msgs::msg::LaserScan>(
        "/scan111", rclcpp::SensorDataQoS(), std::bind(&CostmapGenerator::laserScanCallback, this, std::placeholders::_1));
...
void CostmapGenerator::trackerStatesCallback(const std_msgs::msg::Float32MultiArray::SharedPtr msg) {
    //RCLCPP_INFO(this->get_logger(), "Costmap updated with obstacles");
    ...
    if (!data.empty()) {
        ...
        Updater->ADIC(*msg, obstacles, 0.3);

        publishRawCostmap(msg);
        publishCostmap(msg);
    }
    
    else {
        RCLCPP_WARN(this->get_logger(), "No costmap data available to update.");
    }
}
...
void CostmapGenerator::applyPadding(std::vector<int8_t>& data, int grid_x, int grid_y) {
    // 감쇠 상수 (클수록 빠르게 감쇠함)
    const double decay_factor = 0.8;  // 조절 가능한 파라미터
...
void CostmapGenerator::generateCostmap(const std::vector<int8_t>& data, int width, int height, double resolution) {
    ...
    //     double obstacle_radius = Tracker.getObstacleRadius();
    //     //RCLCPP_INFO(this->get_logger(), "state : %f, %f, %f, %f", state[0], state[1], state[2], state[3]);
    //     std::vector<Obstacle> obstacles = {
    //         Obstacle(state[0], state[1], state[2], state[3], obstacle_radius)
    //     };
