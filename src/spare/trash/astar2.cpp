...
uint AStar::Generator::calculateCost(Vec2i current, Vec2i next) {
    const uint base_value = 5;  //기본 비용, 높으면 대각선 이동 안함
    const double scaling_factor = 0.1; //장애물을 얼마나 무시할지, 높으면 무시함
    ...
    //*No information 상황 예외 처리, G값 튜닝보다는 이게 빠를 것으로 보임, 전역을 장애물로 보기때문에 경로를 못찾음
    if (cell_cost == 255) {
        return base_cost + 1; // cell_cost / 255 = 1
    }
    ...
    while (!openSet.empty()) {
        ...
        for (uint i = 0; i < directions; ++i) {
            ...
            if (successor == nullptr) {
                ...
                // 휴리스틱 함수를 추가하니 경로가 더 목적지 지향적
                successor->G = totalCost + heuristic(successor->coordinates, target_); 
